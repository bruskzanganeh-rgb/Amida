...
// A zone with no DST: one STANDARD component, no RRULE.
function fixedOffsetVTimezone(tzid: string, offset: string, name: string): string {
  return `BEGIN:VTIMEZONE\nTZID:${tzid}\nBEGIN:STANDARD\nTZOFFSETFROM:${offset}\nTZOFFSETTO:${offset}\nTZNAME:${name}\nDTSTART:19700101T000000\nEND:STANDARD\nEND:VTIMEZONE`
}

// VTIMEZONE blocks per supported timezone (RFC 5545 RRULE-based DST rules).
// Must cover every value in TIMEZONE_OPTIONS — see lib/timezones.ts.
const VTIMEZONE_BLOCKS: Record<string, string> = {
  ...
  // South America. Brazil (2019), Argentina, Paraguay (2024), Uruguay, Chile's
  // mainland neighbours: all fixed-offset today — only Santiago still has DST.
  'America/Sao_Paulo': fixedOffsetVTimezone('America/Sao_Paulo', '-0300', 'BRT'),
  'America/Bahia': fixedOffsetVTimezone('America/Bahia', '-0300', 'BRT'),
  'America/Fortaleza': fixedOffsetVTimezone('America/Fortaleza', '-0300', 'BRT'),
  'America/Manaus': fixedOffsetVTimezone('America/Manaus', '-0400', 'AMT'),
  'America/Argentina/Buenos_Aires': fixedOffsetVTimezone('America/Argentina/Buenos_Aires', '-0300', 'ART'),
  'America/Montevideo': fixedOffsetVTimezone('America/Montevideo', '-0300', 'UYT'),
  'America/Asuncion': fixedOffsetVTimezone('America/Asuncion', '-0300', 'PYT'),
  'America/Bogota': fixedOffsetVTimezone('America/Bogota', '-0500', 'COT'),
  'America/Lima': fixedOffsetVTimezone('America/Lima', '-0500', 'PET'),
  'America/La_Paz': fixedOffsetVTimezone('America/La_Paz', '-0400', 'BOT'),
  'America/Caracas': fixedOffsetVTimezone('America/Caracas', '-0400', 'VET'),
