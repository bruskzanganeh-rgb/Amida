...
const statusConfig = {
  draft: { color: 'bg-gray-100 dark:bg-gray-800/40 text-gray-800 dark:text-gray-300' },
  sent: { color: 'bg-blue-100 dark:bg-blue-900/30 text-blue-800 dark:text-blue-300' },
  paid: { color: 'bg-green-100 dark:bg-green-900/30 text-green-800 dark:text-green-300' },
  overdue: { color: 'bg-red-100 dark:bg-red-900/30 text-red-800 dark:text-red-300' },
}
...
export default function InvoicesTab() {
  ...
  const { data: pendingGigs = [], mutate: mutatePendingGigs } = useSWR<PendingGig[]>(
    ...
    async () => {
      ...
      const expensesByGig = expensesData.reduce(
        (acc, expense) => {
    ...
    <PageTransition>
      <div className="space-y-6">
                    ...
                    <TableBody>
                      {pendingGigs.map((gig) => (
                        <TableRow key={gig.id}>
                          ...
                          <TableCell
                            className="text-muted-foreground truncate hidden xl:table-cell"
                            title={gig.project_name || undefined}
                          >
                            {gig.project_name || '-'}
                          </TableCell>
                        ...
                        <div className="min-w-0 flex-1">
                          ...
                          {isSharedMode && invoice.user_id !== currentUserId && (
                            <p className="text-xs text-blue-600 dark:text-blue-400">
                              {getMemberLabel(invoice.user_id)}
                            </p>
                          )}
                        </div>
        ...
        <CreateInvoiceDialog
          ...
          initialGigs={
            selectedPendingGigIds.size > 1 ? pendingGigs.filter((g) => selectedPendingGigIds.has(g.id)) : undefined
          }
        />
        ...
        <ConfirmDialog
          ...
          onOpenChange={(open) => {
            if (!open) setConfirmPaidInvoice(null)
          }}
