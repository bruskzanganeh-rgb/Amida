...
export function EditInvoiceDialog({ invoice, open, onOpenChange, onSuccess, clients }: EditInvoiceDialogProps) {
  ...
  const [pdfSource, setPdfSource] = useState<'sent' | 'original' | null>(null)
  ...
  // Raw display strings for the money fields so they can be cleared and accept
  // in-progress decimals; the field being typed keeps its raw string while the
  // coupled siblings are recomputed.
  ...
  useEffect(() => {
    if (invoice && open) {
      ...
      setSubtotalStr(invoice.subtotal ? String(invoice.subtotal) : '')
      ...
      if (hasSentPdf) {
        loadPdf(invoice.id, 'sent')
      } else if (hasOriginalPdf) {
        loadPdf(invoice.id, 'original')
      } else {
        setPdfUrl(null)
        setPdfSource(null)
      }
  ...
  return (
    <>
      <Dialog open={open} onOpenChange={onOpenChange}>
        <DialogContent className="sm:max-w-lg md:max-w-[900px] max-h-[90vh] flex flex-col">
          <DialogHeader>
          ...
          <div className="overflow-y-auto flex-1 -mx-6 px-6">
            {/* 2-column layout: PDF | Form on desktop, stacked on mobile */}
                  ...
                  <div className="space-y-2">
                    <div className="relative h-80 border rounded-lg overflow-hidden bg-gray-100">
                      <iframe
                        ...
                        title={pdfSource === 'sent' ? t('sentPdf') : t('originalPdf')}
                      />
                      {pdfSource === 'sent' && (
                        <div className="absolute top-2 right-2 pointer-events-none">
                          <span className="bg-blue-600 text-white text-xs font-semibold px-2 py-1 rounded shadow-sm select-none">
                            {t('status.sent')}
                          </span>
                        </div>
                      )}
              ...
              <div className="flex-1 min-w-0 grid grid-cols-1 sm:grid-cols-2 gap-x-4 gap-y-3">
                <div className="space-y-1">
                  ...
                  <Input
                    ...
                    onChange={(e) => {
                      setInvoiceNumberStr(e.target.value)
                      setFormData({ ...formData, invoice_number: parseInt(e.target.value) || 0 })
                    }}
                  />
                </div>
                  ...
                  <div className="flex gap-2">
                    <Input
                      ...
                      onChange={(e) => {
                        ...
                        setFormData({ ...formData, subtotal, vat_amount, total })
                      }}
                    />
                    <Select
                ...
                <div className="space-y-1">
                  ...
                  <Input
                    ...
                    value={vatAmountStr}
                    onChange={(e) => {
                      setVatAmountStr(e.target.value)
                      const vat_amount = parseFloat(e.target.value) || 0
                      const total = round2(formData.subtotal + vat_amount)
                      setTotalStr(String(total))
                      setFormData({ ...formData, vat_amount, total })
                    }}
                  />
                </div>
                ...
                <div className="space-y-1">
                  ...
                  <Input
                    ...
                    onChange={(e) => {
                      ...
                      setVatAmountStr(String(vat_amount))
                      setFormData({ ...formData, total, subtotal, vat_amount })
                    }}
                  />
                </div>
